...
int res_create_localized_surface(const char* name, gr_surface* pSurface, char *setting_locale) {
    char resPath[256];
    GGLSurface* surface = NULL;
    int result = 0;
    unsigned char header[8];
    ...
    *pSurface = NULL;
    ...
    snprintf(resPath, sizeof(resPath)-1, "/res/images/%s/%s_%s.png", setting_locale, setting_locale, name);
    printf("[%s] resPath = [%s]\n", __func__, resPath);
    resPath[sizeof(resPath)-1] = '\0';
	
	if(	access(resPath, F_OK) != 0 )
	{
		snprintf(resPath, sizeof(resPath)-1, "/res/images/en/en_%s.png", name);
	    printf("[%s] resPath = [%s]\n", __func__, resPath);
	    resPath[sizeof(resPath)-1] = '\0';
	}
