...
// TODO : check splash image offset
#define BOOTLOGO_OFFSET     0x00000000
#define PARKINGLINE_OFFSET  0x00150000
#define QUICKBOOT_OFFSET    0x002A0000
#define RESERVED_OFFSET     0x003F0000

enum{
    PRODUCT_DATA_MODEL = 0,
    PRODUCT_DATA_REGION,
    PRODUCT_DATA_OPTION,
    PRODUCT_DATA_VERSUIN,
    PRODUCT_DATA_DATE,
    PRODUCT_DATA_MAX,
};
...
#if 1 //+[TCCQB]
const char *gPartition[PART_MAX] = {"boot", "system", "userdata", "cache",
                                    "recovery", "splash", "kpanic", "misc",
                                    "settings", "dtb", "telesecurity", "snapshot",
                                    "qb_data", "upgrade", "vr"};
#else
const char *gPartition[PART_MAX] = {"boot", "dtb", "system", "userdata",
                                    "cache", "recovery", "splash", "misc",
                                    "settings", "snapshot", "qb_data", "upgrade",
                                    "mymusic", "vr", "data"};
#endif //-[TCCQB]

const char *gSplashPartition[SPLASH_MAX] = {"splash", "logo", "parking", "quickboot"};

static const char *BOOTLOADER_CHECK_FILE = "/upgrade/booterror";
static const char *UPDATE_CHECHSUM_FILE = "/upgrade/checksum";
static const char *UPDATE_RECOVERY_BACKUP_COMPLETE = "/upgrade/up_recovery_backup_compl.txt";
...
static Value* SetMetadataFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if (bad > 0) {
        return ErrorAbort(state, "%s: some changes failed, bad : 0x%08x", name, bad);
    }
...
void getBuildProductData(char *dest, const char *src, char nChar) {
    unsigned int i = 0;
    int j = 0, k = 0, max = PROPERTY_VALUE_MAX;
...
int existVRFile(const char* mFilename)
{
    FILE *_fpN;
    ...
    _fpN = fopen(mFilename, "r");
    if(_fpN != NULL) {
        while(!feof( _fpN )) {
            fgets( strTemp, sizeof(strTemp), _fpN);
            pStr = strstr(strTemp, "vr.img");
             if(pStr != NULL) versionCompare = 1;
        }
        if(_fpN != NULL)
            fclose(_fpN);
    }
...
int splash_write (const char *inputfile, char *outputfile, long int offSet) {
    int ret =0;
    int n,r = 0;
    FILE *fin = NULL, *fout = NULL;
    unsigned char buffer[MAX_BUFF_SIZE];

    fprintf(stdout, "splash_write function entered\n");
    ...
    while (!feof(fin)) {
        n = fread(buffer, 1, MAX_BUFF_SIZE, fin);
        if(ferror(fin)) {
...
int daudio_partition_write (const char *inputfile, char *outputfile) {
...
int daudio_write(const char *partition, const char *filename)
{
    ...
    int splashCnt;
    int ret;
    char mBuildProduct[PRODUCT_DATA_MAX][PROPERTY_VALUE_MAX] = {{0, }};
    char buf[PROPERTY_VALUE_MAX] = {0, };
    char devicePath[PATH_MAX] = {0, };
    char productPath[PATH_MAX] = {0, };
    FILE *file = NULL;
    ...
    for(cnt = 0;cnt < PART_MAX;++cnt) {
        if(!strcmp(partition, gPartition[cnt])) {
            break;
        }
    }

    for(splashCnt = SPLASH_LOGO;splashCnt < SPLASH_MAX;++splashCnt) {
        if(!strcmp(partition, gSplashPartition[splashCnt])) {
            fprintf(stdout, "partition set [%s] -> [splash]\n", partition);
            cnt = PART_SPLASH;
            break;
        }
    }
    ...
    if(cnt >= PART_MAX) {
        fprintf(stderr, "not found [%s] partition\n", partition);
        return -1;
    }

    sprintf(devicePath, "/dev/block/platform/bdm/by-name/%s", gPartition[cnt]);
    fprintf(stdout, "block device path is [%s]\n", devicePath);
    ...
    switch(cnt) {
        ...
        case PART_DTB:
        {
            ret = daudio_partition_write(filename, devicePath);

            if(0 > ret) {
                fprintf(stderr, "[%s] partition data write failed\n", partition);
                return -1;
            }
            break;
        }
        case PART_RECOVERY:
        {
            if(access(UPDATE_RECOVERY_BACKUP_COMPLETE, F_OK)) {
                fprintf(stdout, "start recovery backup update data\n");
                char tempPath[PATH_MAX] = {0, };

                //set backup path
                sprintf(tempPath, "/dev/block/platform/bdm/by-name/%s", gPartition[PART_BOOT]);

                ret = daudio_partition_write(filename, tempPath);
                ...
                char cmd[256];
                sprintf(cmd, "touch %s\n", UPDATE_RECOVERY_BACKUP_COMPLETE);
                system(cmd);
            ...
            remove(UPDATE_RECOVERY_BACKUP_COMPLETE);

            break;
        }
        case PART_VR:
        {
            if(existVRFile(UPDATE_CHECHSUM_FILE) == -1) {
                fprintf(stdout, "There is no VR file !\n");
                return -1;
            }

            memset(buf, 0, PROPERTY_VALUE_MAX);
            memset(mBuildProduct, 0, sizeof(mBuildProduct));

            property_get("ro.build.product", buf, "");
            getBuildProductData((char*)mBuildProduct, buf, '.');

            memset(productPath, 0, PATH_MAX);
            sprintf(productPath, "/usb/DAUDIO/%s/vr.img", mBuildProduct[PRODUCT_DATA_REGION]);
            if(access(productPath, F_OK) != 0) {
                fprintf(stdout, "Does not exist %s\n", productPath);
                return -1;
            }

            fprintf(stdout, "vr Region : %s\n", productPath);
            ret = daudio_partition_write(productPath, devicePath);
        ...
        {
            int splashCnt;
            unsigned int splashOffset;

            for(splashCnt = SPLASH_ORIGIN;splashCnt < SPLASH_MAX;++splashCnt) {
                if(0 == strcmp(partition, gSplashPartition[splashCnt])) {
                    break;
                }
            }
            ...
            switch(splashCnt) {
                case SPLASH_ORIGIN:
                    ...
                    break;
                case SPLASH_LOGO:
                    if(access("sys/bus/usb/devices/1-1/idProduct", F_OK) != 0) {
                        ret = splash_write(filename, devicePath, BOOTLOGO_OFFSET);
                    } else {
                        ret = splash_write("/tmp/logo_bluelink.bmp", devicePath, BOOTLOGO_OFFSET);
                    }
                    ...
                    break;
                case SPLASH_PARKING:
                case SPLASH_QUICKBOOT:
                    if(splashCnt == SPLASH_PARKING) {
                        splashOffset = PARKINGLINE_OFFSET;
                    } else {
                        splashOffset = QUICKBOOT_OFFSET;
                    }

                    ret = splash_write(filename, devicePath, splashOffset);
        ...
        default:
			fprintf(stderr, "[%s] worng partition\n", partition);
            break;
    }
...
Value* WriteRawImageFn(const char* name, State* state, int argc, Expr* argv[])
{
    ...
    //Daudio use eMMC
    fprintf(stdout, "MMC Partition write\n");
    ret = daudio_write(partition, contents->data);
    if(ret == 0) {
        result = strdup(partition);
    } else if (ret == -2) {
        return ErrorAbort(state, "mmc partition write failed (%s), error -2", name);
    } else {
        ErrorAbort(state, "mmc partition write failed (%s)", name);
        fprintf(stdout, "mmc partition write failed (%s)\n", name);
        result = strdup("");
        goto done;
    }
...
Value* WriteSparseImageFn(const char* name, State* state, int argc, Expr* argv[]) {
  bool success = false;
  char* zip_path = NULL;
  char* dest_path = NULL;
  Value* v = malloc(sizeof(Value));
  ...
  if (argc != 2) {
    return ErrorAbort(state, "%s: %s() expects 2 arg, got %d", __func__, name, argc);
  }

  if (ReadArgs(state, argv, 2, &zip_path, &dest_path) < 0)
    return ErrorAbort(state,"%s: %s() Args(zip_path:%s, dest_path:%s)", __func__, name, zip_path, dest_path);
  ...
  /*
   * Start writing sparse image to storage from here!!
   */
  int in, out, ret;
  struct sparse_file *s;

  in = open("/tmp/temp.sparse.img", O_RDWR | O_CREAT, 0664);
  if (in < 0) {
    printf("%s: can't open %s...\n", name, "/tmp/temp.sparse.img");
    while (1) { }
  }
  write(in, v->data, v->size);
  ...
  if (!s) {
    printf("%s: Failed to read sparse file\n", name);
    while (1) { }
  }
  ret = sparse_file_write(s, out, false, false, false);
  if (ret < 0) {
    printf("%s: Can't write output file\n", name);
    while (1) { }
  }
  sparse_file_destroy(s);
  close(in); close(out);
  remove("/tmp/temp.sparse.img");
...
#if 1 // for raw system update
Value* WriteRawZipImageFn(const char* name, State* state, int argc, Expr* argv[]) {
  bool success = false;
  char* zip_path = NULL;
  char* dest_path = NULL;
  Value* v = malloc(sizeof(Value));
  ...
  if (entry == NULL) {
    fprintf(stderr, "%s: no %s in package\n", name, zip_path);
    fprintf(((UpdaterInfo*)(state->cookie))->cmd_pipe, "ui_print %s no %s in package\n", name, zip_path);
    goto done_sparse;
  }
  ...
  success = mzExtractZipEntryToBuffer(za, entry, (unsigned char *)v->data);
  ...
  in = open(dest_path, O_RDWR | O_CREAT, 0664);
  if (in < 0) {
    printf("%s: can't open %s...\n", name, "/tmp/temp.sparse.img");
    while (1) { }
  }
...
Value* WriteSparseImageFnNoZip(const char* name, State* state, int argc, Expr* argv[]) {
    char* src_path = NULL;
    char* dest_path = NULL;
    printf("**WriteSparseImageFnNoZip**\n");

    if(argc != 2) {
        return ErrorAbort(state, "%s: %s() expects 2 arg, got %d", __func__, name, argc);
    }

    if(ReadArgs(state, argv, 2, &src_path, &dest_path) < 0)
        return ErrorAbort(state,"%s: %s() Args(zip_path:%s, dest_path:%s)", __func__, name, src_path, dest_path);
    ...
    */
    ...
    if (in < 0) {
        printf("%s: can't open %s...%s\n", __func__, name, src_path);
        return StringValue(strdup("failure"));
    }

    out = open(dest_path, O_WRONLY);
    if (out < 0) {
        printf("%s: can't open %s...%s\n", __func__, name, dest_path);
        return StringValue(strdup("failure"));
    }
...
Value* WriteBootloaderFn(const char* name, State* state, int argc, Expr* argv[]) {
    ...
    if(ReadArgs(state, argv, 2, &dst_path, &src_path) < 0) {
        return NULL;
    }

    if(strlen(dst_path) == 0 || strlen(src_path) == 0) {
        return ErrorAbort(state, "%s: dst_path(%s) or src_path(%s) argument can't be empty",
            __func__, dst_path, src_path);
    }

    ZipArchive *za = ((UpdaterInfo*)(state->cookie))->package_zip;
    const ZipEntry* entry = mzFindZipEntry(za, src_path);
    if(entry == NULL) {
        return ErrorAbort(state, "%s: failed to find %s from zip file", __func__, src_path);
    }

    v->size = mzGetZipEntryUncompLen(entry);
    v->data = malloc(v->size);
    if(v->data == NULL) {
        return ErrorAbort(state, "%s: failed to allocate %ld bytes for %s",
            __func__, (long)v->size, src_path);
    }

    success = mzExtractZipEntryToBuffer(za, entry, (unsigned char *)v->data);
    if (success == false) {
        return ErrorAbort(state, "%s: failed to extract data from zip file ", __func__);
    }

    int partition_number = 0;
    if(access(BOOTLOADER_CHECK_FILE,F_OK) == 0) {
        fprintf(stdout, "Bootloader has error in the previous update.\n");
        partition_number = 1;
    }

    fprintf(stdout, "1st partition number : %d\n", partition_number);
    result = update_bootloader(partition_number, (const char *)src_path, v->data, v->size);
    if (result < 0) {
        fprintf(stdout, "failed to update bootloader return\n");
        return ErrorAbort(state, "%s: some symlinks failed", name);
    } else {
        if(access(BOOTLOADER_CHECK_FILE,F_OK) == 0) {
            partition_number = 0;
    ...
    fprintf(stdout, "2nd partition number : %d\n", partition_number);
    result = update_bootloader(partition_number, src_path, v->data, v->size);
    if(result < 0) {
        FILE *finz = fopen(BOOTLOADER_CHECK_FILE, "w");
        if(finz == NULL)
            fprintf(stderr, "finz0 == null\n");
        fprintf(stdout, "failed to update bootloader return\n");
        return ErrorAbort(state, "%s: some symlinks failed", name);
    }
    ...
    return success ? StringValue(strdup("success")) : StringValue(strdup("fail"));
}
...
void RegisterInstallFunctions() {
...
#if 1 // for raw system update
    RegisterFunction("write_raw_zip_image", WriteRawZipImageFn);
#endif
    ...
    RegisterFunction("write_sparse_image_no_zip", WriteSparseImageFnNoZip);
    RegisterFunction("write_bootloader", WriteBootloaderFn);
}
