...
int init_mem_code_info(struct mem_code_info *mem, char *src)
{
  ...
  mem->init_code = NULL;
  //mem->init_code = &src[mem->start_code_offset];
#if 1
  mem->init_code = malloc(mem->code_size);
  if (mem->init_code == NULL) {
    printf("Fail to allocate mem_init_code array...\n");
    exit(-1);
  }
  ...
  printf("[%s] mem->code_size         : 0x%08lx\n", __func__, mem->code_size);
  printf("[%s] mem->start_code_offset : 0x%08lx\n", __func__, mem->start_code_offset);
  printf("[%s] mem->end_code_offset   : 0x%08lx\n", __func__, mem->end_code_offset);
...
int init_bootloader_info(struct bootloader_info *bootloader, unsigned int size)
{
  ...
  printf("[%s] Bootloader size : %ld Byte\n", __func__, bootloader->size);
    ...
    struct bootloader_info *bootloader)
{
    ...
    if (memcmp((void*)&h, (void*)&t_h, sizeof(struct boot_header)) != 0) {
        printf("[mmc_update_header] Fail to verify boot header!\n");
        return -1;
    }
    printf("Done.\n");

    printf("============ Boot Header Information ============\n");
    printf("[%s] entry_point         : 0x%08lx\n", __func__, h.entry_point);
    printf("[%s] base_addr           : 0x%08lx\n", __func__, h.base_addr);
    printf("[%s] mem_init_code_size  : 0x%08lx\n", __func__, h.mem_init_code_size);
    printf("[%s] bootloader_size     : 0x%08lx\n", __func__, h.bootloader_size);
    printf("[%s] mem_init_code_base  : 0x%08lx\n", __func__, h.mem_init_code_base);
    printf("[%s] bootloader_base     : 0x%08lx\n", __func__, h.bootloader_base);
    ...
    char *src)
{
  ...
  printf("[2/3 - %s] Update Mem Init Code...", __func__);
  ...
  if (memcmp((void*)mem->init_code, (void*)t_buf, mem->code_size) != 0) {
    printf("[%s] Failed to verify mem init code!\n", __func__);
    return -1;
  }
  printf("Done.\n");
...
int mmc_update_bootloader(
    int dst, 
    struct mem_code_info *mem, 
    struct bootloader_info *bootloader_info, 
    char *src)
{
  printf("[3/3 - %s] Update Bootloader...", __func__);
  ...
  if (memcmp((void*)src, (void*)&t_buf, bootloader_info->size) != 0) {
    printf("[%s] Failed to verify bootloader!\n", __func__);
    return -1;
  }
  printf("Done.\n");
