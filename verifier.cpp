...
#define EOCD_HEADER_SIZE 22
    ...
    if (fseek(fp, -eocd_size, SEEK_END) != 0) {
        LOGE("failed to seek in %s (%s)\n", path, strerror(errno));
        fclose(fp);
        return VERIFY_FAILURE;
    }
    ...
    size_t signed_len = ftell(fp) + EOCD_HEADER_SIZE - 2;
    ...
    if (fread(eocd, 1, eocd_size, fp) != eocd_size) {
...
#define BUFFER_SIZE 4096
    ...
    while (so_far < signed_len) {
        ...
        if (need_sha1) 
			SHA_update(&sha1_ctx, buffer, size);
        if (need_sha256) 
			SHA256_update(&sha256_ctx, buffer, size);
        so_far += size;
    }
    fclose(fp);
    free(buffer);
